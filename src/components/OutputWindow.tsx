...
import './BackgroundPicker.css'; // animated background classes
import './PresentationStyles.css'; // presentation styles and ticker
...
const OutputWindow: React.FC = () => {
  ...
  // Load logo as blob URL whenever logoPath changes
  useEffect(() => {
  ...
  // Global live tick state for Timer Slides
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      await listen('slide-changed', (event: any) => {
        ...
        // Setup initial timer tick manually on push
      ...
      await listen('blank-toggled', (event: any) => {
        const info: PresentationInfo = event.payload;
        setIsBlank(info.is_blank);
      });

      await listen('presentation-started', (event: any) => {
        const info: PresentationInfo = event.payload;
        setCurrentSlide(info.current_slide);
        setIsBlank(info.is_blank);
        setIsLive(true);
      ...
      await listen('presentation-stopped', (_event: any) => {
        setIsLive(false);
        setIsBlank(false);
      });
      
      // Listen to tick directly from Rust timer
      ...
      // Update local settings when changed in control window
  ...
  return (
    <div className={`output-window style-${settings.presentationStyle}`}>
      ...
      {/* BBC-style channel logo watermark top-left */}
      {logoUrl && (
        <div className="church-watermark">
          <img src={logoUrl} alt="Church Logo" className="watermark-logo" />
          {settings.churchName && <span className="watermark-name">{settings.churchName}</span>}
